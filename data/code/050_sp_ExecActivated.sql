CREATE OR ALTER PROCEDURE async.sp_ExecActivated
AS
BEGIN
            ...
            BEGIN
                IF (@messageTypeName = N'DEFAULT')
                    BEGIN
                        ...
                        IF (@sentence IS NOT NULL AND TRIM(@sentence) <> '')
                            BEGIN
                                ...
                                BEGIN CATCH
                                    ...
                                    IF (@xactState = -1)
                                        BEGIN
                                            RAISERROR (N'[async].[sp_ExecActivated]: Unrecoverable error in procedure %s: %i: %s', 16, 10,
                                                @sentence, @execErrorNumber, @execErrorMessage);
                                        END
                                    ELSE
                                        IF (@xactState = 1)
                                            BEGIN
                                                PRINT 'Error [async].[sp_ExecActivated] = 1'
                                            END
                                END CATCH
                        ...
                        IF (@queued_id IS NULL)
                            BEGIN
                                RAISERROR (N'[async].[sp_ExecActivated]: Internal consistency error: conversation not found', 16, 20);
                            END
                        ...
                        IF (0 = @@ROWCOUNT)
                            BEGIN
                                RAISERROR (N'[async].[sp_ExecActivated]: Internal consistency error: token not found', 16, 30);
                            END
                ...
                ELSE
                    ...
                    ELSE
                        ...
                        ELSE
                            BEGIN
                                RAISERROR (N'[async].[sp_ExecActivated]: Received unexpected message type: %s', 16, 50, @messageTypeName);
                            END
            END
    ...
    BEGIN CATCH
        ...
        RAISERROR (N'[async].[sp_ExecActivated]: Error: %i, %s', 1, 60, @error, @message) WITH LOG;
